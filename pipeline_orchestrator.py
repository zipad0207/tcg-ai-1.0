...
3. **践行【缺啥补啥】原则**：
   ...
   - 双色卡（红蓝 4xx、蓝绿 5xx、红绿 6xx）互相融合两色机制，形成互补闭环。
4. **【核心准则：机制与特色词条驱动（严禁全员白板，绝对红线）】**：
   - **机制驱动指标**：扩展包必须以战术机制为核心驱动！三大阵营每个阵营 6 张新卡中，**必须有至少 5 张携带 1~2 个合法战术词条**；
   - **纯白板严格限额**：纯白板随从（tags 为 []）每个阵营全套扩展包**最多仅允许 0~1 张**（用于填补特定费用扎实身材），绝对严禁批量产出无词条白板废卡！
   - **中立通用卡 100% 机制化**：3 张中立通用卡必须全部携带功能词条（如过牌 DRAW、坚守 FORTIFY、临时法力 TEMP_MANA 等通用润滑机制）；
   - **严禁卡牌重名**：新卡名称必须是全新、独立的中文命名，绝对严禁直接复用已有卡牌名称！
   - 合法词条清单（仅限以下 14 种）：
...
### 3. 输出格式要求：
必须严格输出纯 JSON 对象，格式示范如下（注意 tags 字段必须按战术规则合理填充）：
{{
  ...
  "factions": {{
    "Red": [
      ...
      ... (共 6 张: 4 单位 + 2 法术，含 1 张双色卡，至少 5 张具备合法战术词条)
    ],
    "Blue": [
      ... (共 6 张: 4 单位 + 2 法术，含 1 张双色卡，至少 5 张具备合法战术词条)
    ],
    ...
    "Neutral": [
      ... (共 3 张中立通用卡: 全部具备合法功能词条，factions 为 [\"Neutral\"])
    ]
  }}
}}
  }}
