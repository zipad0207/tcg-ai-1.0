...
7. **【核心准则：机制与特色词条驱动（严禁全员白板，绝对红线）】**：
   - **机制驱动指标**：设计出的新卡中，**必须有至少 85% 以上携带 1~2 个合法战术词条**，突出该阵营的核心机制优势；
   - **纯白板严格限额**：全套新卡中纯白板随从（tags 为 []）**最多仅允许 0~1 张**，严禁批量产出无词条白板废卡！
   - **严禁卡牌重名**：新卡名称必须是全新、独立的中文命名，绝对严禁直接复用现有参考卡池中的已有卡名！
