...
7. **严禁高费亏模白板（Anti-Understatted-High-Cost-Vanilla Rule，绝对红线）**：
   - 严禁调出或保留「高费用（>= 5 费），身材严重亏模且无任何词条」的垃圾白板废卡（例如 7~8 费却只有 4~5 DP 且没有任何词条，绝对不可接受）！
   - 高费卡牌（>= 5 费）必须满足以下二选一原则：
     a. 机制/词条驱动型：携带强力或功能性词条（如 RUSH、DEGRADE、FORTIFY、BONUS_SCORE、DEATH_DRAW 等），身材可适度控制在 4~6 DP；
     b. 纯白板身材补偿型：若无任何词条（纯白板），其身材必须严格遵守高费白板超模补偿模型（参考底线：5费白板 >= 6 DP，6费白板 >= 7~8 DP，7费白板 >= 8~9 DP，8费白板 >= 9~10 DP）。
   - 平衡调整关键准则：若为了抑制某张高费强势卡而剥离了其突袭等核心词条，绝不能把身材留在低数值让其沦为高费白板废卡！若去掉词条，必须同步补足基础身材；若压低身材，必须保留功能性机制。
