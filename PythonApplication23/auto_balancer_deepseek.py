...
---
### 调整要求：
1. 目标胜率平抑：当前红方胜率为 {p0_rate:.1f}%，蓝方胜率为 {p1_rate:.1f}%。请仅挑选 2~3 张关键卡进行微调：
   - 适度回调调整过度的卡牌；
   - 避免大幅改动其他平衡卡，力求双方胜率接近 50% 平衡线。
2. 严格保持现有卡牌名称与数量完全一致，不要新增或删除卡牌。
3. 必须只输出合法且可直接解析的纯 JSON 字符串，不要包含解释文本。
"""
