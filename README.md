...
1. **平衡目标**：三大阵营相对 50% 基准线的**最大偏离度 $\le 2.80\%$（即胜率全部落在 $[47.2\%, 52.8\%]$ 平衡区间）**，经 4 轮微调（R0 $\to$ R3）达成平衡。
...
| **整体统计** | **3,000 局** | **3,000 场** | **最大偏离度: 2.09%** ($\le 2.80\%$ 目标阈值) | | **各阵营胜率均达到平衡标准** |
...
├── auto_balancer_deepseek.py            # 基于对战数据的 LLM 卡牌数值针对性微调工具
