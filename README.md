...
## ⚡ 极速体验：免安装绿色运行整合包（小白开箱即用）

如果您不需要配置复杂的本地 Python 开发环境，可直接下载打包好的**纯净无权重便携版**：

- 📥 **网盘下载地址**：[点击前往网盘下载](https://pan.baidu.com/) （请在此替换为您的网盘分享链接，提取码：xxxx）
- 📦 **版本特性**：
  - **100% 独立离线**：内嵌专属 Python 运行环境与 PyTorch、FastAPI 等全套依赖，断网也能直接秒开。
  - **纯净无权重**：未预置任何收敛模型权重，确保从零开始客观验证“卡牌生成 ➔ 强化学习自博弈 ➔ 数值自动平衡”全生命周期流程。
- 🚀 **使用方法**：
  1. 下载后**完整解压**到任意文件夹（建议避免中文路径）。
  2. 双击运行根目录下的 **`run_web.bat`**，系统将自动唤起默认浏览器打开可视化控制台（`http://127.0.0.1:8000`）。
