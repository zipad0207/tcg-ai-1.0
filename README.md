...
* **调优后 6,000 局终极均衡态 (双维度统计)**：

| 阵营流派 | 外战总场次 | 外战胜场 | **纯外战胜率 (Non-Mirror)** | 综合总场次 (含内战) | **综合胜率 (Overall)** |
| :--- | :---: | :---: | :---: | :---: | :---: |
| 🔴 **赤红 (Red)** | 2,723 | 1,186 | **43.56%** | 4,033 | **45.65%** |
| 🔵 **蔚蓝 (Blue)** | 2,602 | 1,551 | **59.61%** | 3,950 | **56.33%** |
| 🟢 **翠绿 (Green)** | 2,641 | 1,246 | **47.18%** | 4,017 | **48.15%** |

* 🏆 **三大阵营双向交锋 3x3 对弈矩阵 (对角线内战为 50.0% 严格基准)**：

| 本方 \ 对手 | 🔴 赤红 (Red) | 🔵 蔚蓝 (Blue) | 🟢 翠绿 (Green) | 战术交锋与先手表现 |
| :--- | :---: | :---: | :---: | :--- |
| 🔴 **赤红 (Red)** | **50.0%** *(先手 52.1% / 后手 47.9%)* | **38.9%** | **48.1%** | 面对控制型蓝方防线较吃力，与翠绿平分秋色 |
| 🔵 **蔚蓝 (Blue)** | **61.1%** | **50.0%** *(先手 51.6% / 后手 48.4%)* | **58.0%** | 护甲防线成型后对快攻和中慢速均有稳健压制 |
| 🟢 **翠绿 (Green)** | **51.9%** | **42.0%** | **50.0%** *(先手 51.7% / 后手 48.3%)* | 前期护脸补强后抗住红方快攻，与红蓝均可一战 |

> **关键统计纠偏说明**：
> 1. **排除内战 100% 统计 Bug**：先前在原始数据结构中，由于内战双方阵营名相同（`name0 == name1`），代码将双方胜场累计到了同一个键名，造成原始字典出现“自己打自己胜率 100%”的假象。现已彻底修正为内战分立 `p0_first_wins`（先手胜场）与 `p1_second_wins`（后手胜场），内战双方综合胜率严格为 **50.0%**（数学零和博弈）。
> 2. **外战胜率 vs 综合胜率**：在 TCG（如炉石、万智牌）大数据中，内战局数约占 1/3（本实验 2,017 场），会天然将两极胜率向 50% 拉平。“纯外战胜率”真实反映了阵营间的克制链，而“综合胜率”反映了排位大盘的总体表现，两者结合为学术答辩提供完整的量化闭环。
